...
export function RecommendationCard({ recommendation, plan, onSelect, onCompare }: RecommendationCardProps) {
  ...
  return (
    <Card className="relative">
        ...
        <div className="flex items-start justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              {plan && (
                <Badge variant="secondary">{plan.supplierName}</Badge>
              )}
            </CardTitle>
            {plan && (
      ...
      <CardContent className="space-y-4">
          ...
          {plan && (
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div>
                ...
                <span className="font-medium">{formatCurrency(plan.ratePerKwh)}/kWh</span>
              </div>
              <div>
                <span className="text-muted-foreground">Type:</span>{' '}
                <span className="font-medium capitalize">{plan.contractType}</span>
              </div>
              {plan.contractLengthMonths && (
                <div>
                  <span className="text-muted-foreground">Contract:</span>{' '}
                  <span className="font-medium">{plan.contractLengthMonths} months</span>
                </div>
              )}
              {plan.renewablePercentage !== undefined && (
                <div>
                  <span className="text-muted-foreground">Renewable:</span>{' '}
                  <span className="font-medium">{plan.renewablePercentage}%</span>
                </div>
              )}
            </div>
          )}
        ...
        <div>
          <h4 className="text-sm font-medium mb-2">Why This Plan?</h4>
